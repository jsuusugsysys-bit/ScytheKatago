...
public class ReadBoard {
  ...
  public ReadBoard(boolean usePipe, boolean isJavaReadBoard) throws Exception {
    ...
    // usePipe=true 时工作目录会被设为 readboard，所以命令只需要 exe 名
    if (usePipe) engineCommand = "readboard.exe";
  ...
  public void parseLine(String line) {
    // 诊断日志：记录所有收到的命令
    DebugLogger.logReadBoard("parseLine 收到: " + line);
    ...
    // 处理镰刀触发命令
    if (line.startsWith("scythe_trigger")) {
      String[] params = line.trim().split(" ");
      if (params.length >= 2 && Lizzie.leelaz != null && Lizzie.leelaz.isKatago) {
        String color = params[1]; // "black" 或 "white"
        System.err.println("ReadBoard: Scythe triggered for " + color);

        // 判断是否是 AI 方的镰刀
        // AI 执白时，黑方镰刀 = 对手镰刀（isAi=false）
        // AI 执黑时，白方镰刀 = 对手镰刀（isAi=false）
        boolean isBlackScythe = color.equals("black");
        boolean aiPlaysBlack =
            LizzieFrame.toolbar.isAutoPlay && LizzieFrame.toolbar.chkAutoPlayBlack.isSelected();
        boolean aiPlaysWhite =
            LizzieFrame.toolbar.isAutoPlay && LizzieFrame.toolbar.chkAutoPlayWhite.isSelected();

        // 判断这是 AI 方的镰刀还是对手方的镰刀
        boolean isAiScythe = (isBlackScythe && aiPlaysBlack) || (!isBlackScythe && aiPlaysWhite);

        System.err.println(
            "ReadBoard: isBlackScythe="
                + isBlackScythe
                + ", aiPlaysBlack="
                + aiPlaysBlack
                + ", aiPlaysWhite="
        ...
        // 立即减少镰刀计数并设置连击状态
        String playerColor = isBlackScythe ? "B" : "W";

        // 1. 查询当前镰刀数量
        if (LizzieFrame.scythePanel != null) {
          int currentCount =
              isBlackScythe
                  ? LizzieFrame.scythePanel.getBlackScythes()
                  : LizzieFrame.scythePanel.getWhiteScythes();

          if (currentCount > 0) {
            // 2. 减少镰刀数量
            int newCount = currentCount - 1;
            Lizzie.leelaz.sendCommand("kata-set-scythe-count " + playerColor + " " + newCount);
            System.err.println(
                "ReadBoard: 减少镰刀数量 " + playerColor + ": " + currentCount + " -> " + newCount);

            // 3. 立即设置连击状态（在对手落第1手之前）
            // 使用 kata-set-param scythe_combo_next <color> 告诉引擎下一手会触发镰刀连击
            // 或者直接通过 ScythePanel 触发（它会发送 scythe_trigger）
            LizzieFrame.scythePanel.triggerScythe(color, isAiScythe);
            System.err.println("ReadBoard: ScythePanel.triggerScythe() called - 引擎将在下一手落子时触发连击");
          } else {
            System.err.println("ReadBoard: 警告 - " + playerColor + " 没有剩余镰刀！");
          }
        }
      }
    }
    ...
    if (line.startsWith("both")) {
      DebugLogger.logReadBoard("收到 bothSync 命令，设置 bothSync=true, 之前值=" + Lizzie.frame.bothSync);
      Lizzie.frame.bothSync = true;
      DebugLogger.logReadBoard("bothSync 已设置为 true, 当前值=" + Lizzie.frame.bothSync);
    ...
    if (line.startsWith("play")) {
      ...
      if (params.length == 3) {
        ...
        if (time > 0) {
          ...
          LizzieFrame.toolbar.txtAutoMain.setText(String.valueOf(time)); // 同步到 txtAutoMain
          ...
          LizzieFrame.toolbar.txtAutoMain.setText(
              String.valueOf(
                  Lizzie.config.leelazConfig.getInt(
                      "max-game-thinking-time-seconds"))); // 同步到 txtAutoMain
          LizzieFrame.toolbar.chkAutoPlayTime.setSelected(true);
        }
        if (playouts > 0) {
  ...
  /**
   * 检查 readboard 是否准备好接收命令
   *
   * @return true 如果可以发送命令
   */
  public boolean isReady() {
    if (usePipe) {
      return process != null && process.isAlive();
    } else {
      return readBoardStream != null;
    }
  }
}
