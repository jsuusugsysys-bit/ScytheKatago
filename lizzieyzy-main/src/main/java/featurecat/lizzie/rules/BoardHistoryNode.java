...
public class BoardHistoryNode {
  ...
  public void clearAndSyncBoard(boolean stepIn) {
    if (stepIn) {
      ...
      // Scythe: Save scythe state before clear_board
      int savedBlackScythes = 3;
      int savedWhiteScythes = 3;
      if (LizzieFrame.scythePanel != null) {
        savedBlackScythes = LizzieFrame.scythePanel.getBlackScythes();
        savedWhiteScythes = LizzieFrame.scythePanel.getWhiteScythes();
      }

      Lizzie.leelaz.clear();

      // Scythe: Restore scythe state after clear_board (only for 11x11 board)
      if (Board.boardWidth == 11 && Board.boardHeight == 11) {
        Lizzie.leelaz.sendCommand("kata-set-param scythe_count_black " + savedBlackScythes);
        Lizzie.leelaz.sendCommand("kata-set-param scythe_count_white " + savedWhiteScythes);
      }
