...
namespace readboard
{
    public partial class MainForm : Form
    {
        ...
        // lw.lwsoft lw; // lw COM 组件已移除，使用前台模式
        object lwDummy = null;
        ...
        private void stopKeepingSync()
        {
            ...
            startedSync = false;
            // lw 后台模式已移除，跳过 ForceUnBindWindow
        ...
        private void OutPutTime()
        {
            ...
            while (keepSync)
            {
                ...
                // === 镰刀检测（每次同步循环都检测）===
                if (scytheDetectEnabled && type == 0)  // 仅野狐模式
                {
                    try
                    {
                        // 获取野狐窗口完整截图
                        Bitmap fullWindowScreenshot = GetWindowImage(hwnd);
                        if (fullWindowScreenshot != null)
                        {
                            DetectScytheTrigger(fullWindowScreenshot);
                            fullWindowScreenshot.Dispose();
                        }
                    }
                    catch (Exception ex)
                    {
                        System.Diagnostics.Debug.WriteLine("镰刀检测异常: " + ex.Message);
                    }
                }
            ...
            for (int i = 0; i < boardH; i++)
            {
                for (int j = 0; j < boardW; j++)
                {
                    ...
                    if (resultHere == 1 || resultHere == 3)
                        allWhite = false;
                    // 记录最后一手的颜色（用于镰刀检测）
                    if (resultHere == 3)  // 黑子最后一手
                        lastMoveWasBlack = true;
                    else if (resultHere == 4)  // 白子最后一手
        ...
        private void OutPut(Boolean first, RgbInfo[] rgbArray, object lwh, int totalWidth)
        {
            if (width < this.boardW || height < this.boardH)
                return;
            // lw 双向同步功能已移除（需要 lw COM 组件）
        ...
        // 镰刀检测状态
        private bool scytheDetectEnabled = false;
        private int blackScytheCount = 3;
        private int whiteScytheCount = 3;
        private int scytheProgress = 0;  // 0-3: 当前连续落子进度
        private bool isAIBlack = true;   // AI 执黑还是执白
        private double aiWinrate = 0.0;  // AI 胜率

        // 防抖和冷却机制
        private int blackConsecutiveMatches = 0;  // 黑方连续匹配次数
        private int whiteConsecutiveMatches = 0;  // 白方连续匹配次数
        private DateTime lastBlackTriggerTime = DateTime.MinValue;  // 上次黑方触发时间
        private DateTime lastWhiteTriggerTime = DateTime.MinValue;  // 上次白方触发时间
        private const int REQUIRED_MATCHES = 3;   // 需要连续匹配3次才触发
        private const int COOLDOWN_SECONDS = 5;   // 冷却时间5秒
        private const int PIXEL_THRESHOLD = 20;   // 像素阈值（黄色框框不大，降低到20）
        private int lastDebugSecond = -1;         // 上次调试输出的秒数
        private bool lastMoveWasBlack = true;     // 最后一手是否为黑棋（用于判断镰刀触发方）
        private int scytheTextDetectCount = 0;    // "本回合走3步"检测计数
        private bool lastScytheTextState = false; // 上一次是否检测到镰刀文字（用于边沿触发）
        private bool scytheTriggeredThisRound = false; // 本轮是否已触发（防止重复触发）
        private int blackStoneCount = 0;          // 棋盘上黑子数量
        private int whiteStoneCount = 0;          // 棋盘上白子数量

        // 镰刀文字颜色阈值（从 Java 版本移植）
        // 白方镰刀：红色文字 (R高, G低, B低)
        private const int WHITE_SCYTHE_RED_MIN = 180;
        private const int WHITE_SCYTHE_GREEN_MAX = 100;
        private const int WHITE_SCYTHE_BLUE_MAX = 100;
        // 黑方镰刀：蓝紫色文字 (R中低, G低, B高)
        private const int BLACK_SCYTHE_BLUE_MIN = 150;
        private const int BLACK_SCYTHE_RED_MAX = 180;
        private const int BLACK_SCYTHE_GREEN_MAX = 120;

        // 镰刀检测启用/禁用
        private void chkScytheDetect_CheckedChanged(object sender, EventArgs e)
        {
            scytheDetectEnabled = chkScytheDetect.Checked;
            numBlackScythe.Enabled = scytheDetectEnabled;
            numWhiteScythe.Enabled = scytheDetectEnabled;
            lblScytheProgress.Visible = scytheDetectEnabled;

            if (scytheDetectEnabled)
            {
                // 初始化镰刀状态
                blackScytheCount = (int)numBlackScythe.Value;
                whiteScytheCount = (int)numWhiteScythe.Value;
                scytheProgress = 0;
                UpdateScytheProgressDisplay();
                UpdateAIWinrateDisplay();
            }
        }

        // 黑方镰刀数量手动调整
        private void numBlackScythe_ValueChanged(object sender, EventArgs e)
        {
            blackScytheCount = (int)numBlackScythe.Value;
            // 同步到 lizzieyzy
            if (scytheDetectEnabled && useTcp)
            {
                Send("kata-set-param scythe_count_black " + blackScytheCount);
            }
        }

        // 白方镰刀数量手动调整
        private void numWhiteScythe_ValueChanged(object sender, EventArgs e)
        {
            whiteScytheCount = (int)numWhiteScythe.Value;
            // 同步到 lizzieyzy
            if (scytheDetectEnabled && useTcp)
            {
                Send("kata-set-param scythe_count_white " + whiteScytheCount);
            }
        }

        // 更新进度圆圈显示
        private void UpdateScytheProgressDisplay()
        {
            string circles = "";
            for (int i = 0; i < 3; i++)
            {
                circles += (i < scytheProgress) ? "●" : "○";
            }
            lblScytheProgress.Text = "进度:" + circles;
        }

        // 更新 AI 胜率显示
        private void UpdateAIWinrateDisplay()
        {
            string colorStr = isAIBlack ? "黑" : "白";
            if (aiWinrate > 0)
            {
                lblAIWinrate.Text = string.Format("AI({0}) 胜率: {1:F1}%", colorStr, aiWinrate);
            }
            else
            {
                lblAIWinrate.Text = string.Format("AI({0}) 胜率: --.-%%", colorStr);
            }
        }

        // 检测镰刀触发（检测野狐棋盘上方的"本回合走3步"文字）
        // 新方案：检测棋盘上方区域的白色文字，然后用最后一手颜色判断是谁触发镰刀
        private void DetectScytheTrigger(Bitmap screenshot)
        {
            if (!scytheDetectEnabled || screenshot == null) return;

            int width = screenshot.Width;
            int height = screenshot.Height;

            // 检测区域：棋盘上方的"本回合走3步"文字区域
            // 位置：顶部 2%-8% 高度，横向 20%-80% 宽度
            int textStartX = (int)(width * 0.20);
            int textEndX = (int)(width * 0.80);
            int textStartY = (int)(height * 0.02);
            int textEndY = (int)(height * 0.08);

            int whitePixelCount = 0;  // 白色文字像素计数
            int totalPixels = 0;

            // 扫描"本回合走3步"文字区域，检测白色像素
            for (int y = textStartY; y < textEndY; y += 2)
            {
                for (int x = textStartX; x < textEndX; x += 2)
                {
            ...
            // 计算白色像素密度
            double whiteRatio = totalPixels > 0 ? (double)whitePixelCount / totalPixels : 0;

            // 镰刀检测阈值：当白色像素比例超过 3% 时认为出现了"本回合走3步"
            // 实测：镰刀出现时约 4.74%，正常时约 0%
            const double SCYTHE_TEXT_THRESHOLD = 0.03;
            bool scytheTextDetected = whiteRatio > SCYTHE_TEXT_THRESHOLD;

            // 写入日志文件（每秒输出一次）
            if (DateTime.Now.Second != lastDebugSecond)
            {
                lastDebugSecond = DateTime.Now.Second;
                string logLine = string.Format("{0} [检测] 白字:{1}/{2} 比例:{3:F2}% 阈值:{4:F1}% 最后手:{5} 窗口:{6}x{7}",
                    DateTime.Now.ToString("HH:mm:ss"),
                    whitePixelCount, totalPixels,
                    whiteRatio * 100, SCYTHE_TEXT_THRESHOLD * 100,
                    lastMoveWasBlack ? "黑" : "白",
                    width, height);
                WriteScytheLog(logLine);
            }
            ...
            try
            {
                this.Invoke((MethodInvoker)delegate {
                    lblScytheProgress.Text = string.Format("白字{0:F1}%", whiteRatio * 100);
                });
            }
            catch { }

            // 边沿触发机制：只有从"未检测到"变成"检测到"时才触发一次
            // 防止同一次镰刀被重复触发

            if (scytheTextDetected)
            {
                scytheTextDetectCount++;

                // 如果本轮已经触发过，不再重复触发
                if (scytheTriggeredThisRound)
                {
                    // 每秒只输出一次日志
                    if (DateTime.Now.Second != lastDebugSecond)
                    {
                        WriteScytheLog(string.Format("{0} [等待] 本轮已触发，等待镰刀文字消失... 黑剩:{1} 白剩:{2}",
                            DateTime.Now.ToString("HH:mm:ss"), blackScytheCount, whiteScytheCount));
                    }
                    return;
                }

                // 详细日志：追踪防抖计数
                WriteScytheLog(string.Format("{0} [防抖] 检测到! 计数:{1}/{2} 最后手:{3} 黑剩:{4} 白剩:{5}",
                    DateTime.Now.ToString("HH:mm:ss"), scytheTextDetectCount, REQUIRED_MATCHES,
                    lastMoveWasBlack ? "黑" : "白", blackScytheCount, whiteScytheCount));
            }
            else
            {
                // 镰刀文字消失了，重置状态，允许下次触发
                if (scytheTriggeredThisRound)
                {
                    WriteScytheLog(string.Format("{0} [重置] 镰刀文字消失，允许下次触发",
                        DateTime.Now.ToString("HH:mm:ss")));
                    scytheTriggeredThisRound = false;
                }
                if (scytheTextDetectCount > 0)
                {
                    WriteScytheLog(string.Format("{0} [防抖] 重置计数 (之前:{1})",
                        DateTime.Now.ToString("HH:mm:ss"), scytheTextDetectCount));
                }
                scytheTextDetectCount = 0;
                return;
            }

            // 需要连续检测到多次才触发
            if (scytheTextDetectCount >= REQUIRED_MATCHES)
            {
                // 检测右侧黄色框位置来判断是谁触发镰刀
                // 野狐布局：右侧镰刀区域，白棋在上，黑棋在下
                // 触发镰刀时，对应方的框会被黄色高亮
                bool isBlackScythe = DetectYellowBoxPosition(screenshot, width, height);

                WriteScytheLog(string.Format("{0} [触发条件] 黄色框检测 → 判断:{1}方镰刀 剩余:{2}",
                    DateTime.Now.ToString("HH:mm:ss"),
                    isBlackScythe ? "黑" : "白",
                    isBlackScythe ? blackScytheCount : whiteScytheCount));

                if (isBlackScythe && blackScytheCount > 0)
                {
                    scytheTriggeredThisRound = true;  // 标记本轮已触发
                    scytheTextDetectCount = 0;

                    WriteScytheLog(string.Format("{0} [触发!] 黑方镰刀",
                        DateTime.Now.ToString("HH:mm:ss")));

                    TriggerScythe(true);  // 黑方镰刀
                }
                else if (!isBlackScythe && whiteScytheCount > 0)
                {
                    scytheTriggeredThisRound = true;  // 标记本轮已触发
                    scytheTextDetectCount = 0;

                    WriteScytheLog(string.Format("{0} [触发!] 白方镰刀",
                        DateTime.Now.ToString("HH:mm:ss")));

                    TriggerScythe(false);  // 白方镰刀
                }
                else
                {
                    WriteScytheLog(string.Format("{0} [无法触发] {1}方镰刀剩余为0",
                        DateTime.Now.ToString("HH:mm:ss"), isBlackScythe ? "黑" : "白"));
                }
            }
        }

        // 检测黄色框位置，判断是黑方还是白方触发镰刀
        // 野狐布局：右侧镰刀区域，白棋在上（约15%-35%高度），黑棋在下（约35%-55%高度）
        // 返回：true = 黑方镰刀，false = 白方镰刀
        private bool DetectYellowBoxPosition(Bitmap screenshot, int width, int height)
        {
            // 右侧镰刀指示器区域（窗口右侧约 8% 宽度）
            int indicatorStartX = (int)(width * 0.90);
            int indicatorEndX = width - 5;

            // 白棋区域在上半部分（约 15%-35% 高度）
            int whiteStartY = (int)(height * 0.15);
            int whiteEndY = (int)(height * 0.35);

            // 黑棋区域在下半部分（约 35%-55% 高度）
            int blackStartY = (int)(height * 0.35);
            int blackEndY = (int)(height * 0.55);

            int whiteYellowPixels = 0;
            int blackYellowPixels = 0;

            // 扫描白棋区域的黄色像素
            for (int y = whiteStartY; y < whiteEndY; y += 2)
            {
                for (int x = indicatorStartX; x < indicatorEndX; x += 2)
                {
                    if (x >= 0 && x < width && y >= 0 && y < height)
                    {
                        Color pixel = screenshot.GetPixel(x, y);
            ...
            // 扫描黑棋区域的黄色像素
            for (int y = blackStartY; y < blackEndY; y += 2)
            {
                for (int x = indicatorStartX; x < indicatorEndX; x += 2)
                {
                    if (x >= 0 && x < width && y >= 0 && y < height)
            ...
            WriteScytheLog(string.Format("{0} [黄色框] 白区:{1} 黑区:{2}",
                DateTime.Now.ToString("HH:mm:ss"), whiteYellowPixels, blackYellowPixels));

            // 哪个区域黄色像素更多，就是哪方触发镰刀
            return blackYellowPixels > whiteYellowPixels;
        }

        // 触发镰刀
        private void TriggerScythe(bool isBlack)
        {
            string colorStr = isBlack ? "black" : "white";
            string colorChn = isBlack ? "黑方" : "白方";

            // 写入日志文件
            WriteScytheLog("========================================");
            WriteScytheLog(string.Format("{0} [触发] {1}镰刀! 命令: scythe_trigger {2}",
                DateTime.Now.ToString("HH:mm:ss"), colorChn, colorStr));
            WriteScytheLog("========================================");

            if (isBlack)
            {
                blackScytheCount--;
                this.Invoke((MethodInvoker)delegate { numBlackScythe.Value = blackScytheCount; });
            }
            else
            {
                whiteScytheCount--;
                this.Invoke((MethodInvoker)delegate { numWhiteScythe.Value = whiteScytheCount; });
            }

            // 发送触发命令到 lizzieyzy
            if (useTcp)
            {
                Send("scythe_trigger " + colorStr);
            }
            ...
            this.Invoke((MethodInvoker)delegate {
                UpdateScytheProgressDisplay();
                // 在进度标签显示触发信息（不弹窗）
                lblScytheProgress.Text = "触发:" + colorChn + " 剩:" + blackScytheCount + "/" + whiteScytheCount;
            });
        }

        // 写入镰刀检测日志
        private void WriteScytheLog(string message)
        {
            try
            {
                string logPath = System.IO.Path.Combine(Application.StartupPath, "scythe_detect.log");
        ...
        // 更新镰刀进度（每次落子后调用）
        private void UpdateScytheProgress()
        {
            if (!scytheDetectEnabled) return;

            if (scytheProgress > 0 && scytheProgress < 3)
            {
                scytheProgress++;
                UpdateScytheProgressDisplay();
            }
            else if (scytheProgress >= 3)
            {
                scytheProgress = 0;
                UpdateScytheProgressDisplay();
            }
        ...
        // 设置 AI 胜率（从 lizzieyzy 接收）
        public void SetAIWinrate(double winrate, bool isBlack)
        {
            aiWinrate = winrate;
            isAIBlack = isBlack;
            if (this.InvokeRequired)
            {
                this.Invoke(new Action(() => UpdateAIWinrateDisplay()));
            }
            else
            {
                UpdateAIWinrateDisplay();
