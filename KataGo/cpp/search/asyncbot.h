...
class AsyncBot {
 public:
  ...
  void setManualScytheTrigger(bool value, Player triggerPlayer = C_EMPTY);
  void setScytheCount(Player pla, int count);
  void setScytheState(int blackScythes, int whiteScythes, int combo, bool manualTrigger, Player comboPlayer = C_EMPTY);
  // Scythe: Activate combo immediately for analysis (sets combo, comboPlayer, presumedNextMovePla)
  void activateScytheCombo(Player triggerPla, int combo);

  // Scythe: Generate 3-move sequence for scythe combo
  // Activates scythe, performs 3 sequential searches, returns the move locations
  std::vector<Loc> scytheGenmove(Player pla, const TimeControls& tc);
