...
// Scythe: Set manual trigger flag
void AsyncBot::setManualScytheTrigger(bool value, Player triggerPlayer) {
  stopAndWait();
  search->rootHistory.manualScytheTrigger = value;
  search->rootHistory.manualScytheTriggerPlayer = triggerPlayer;
}
...
void AsyncBot::setScytheState(int blackScythes, int whiteScythes, int combo, bool manualTrigger, Player comboPlayer) {
  stopAndWait();
  search->rootHistory.blackScythes = blackScythes;
  search->rootHistory.whiteScythes = whiteScythes;
  search->rootHistory.scytheCombo = combo;
  search->rootHistory.scytheComboPlayer = comboPlayer;
  search->rootHistory.manualScytheTrigger = manualTrigger;
}

void AsyncBot::activateScytheCombo(Player triggerPla, int combo) {
  stopAndWait();
  // Decrement scythe count
  if(triggerPla == P_BLACK)
    search->rootHistory.blackScythes--;
  else
    search->rootHistory.whiteScythes--;
  // Set combo state
  search->rootHistory.scytheCombo = combo;
  search->rootHistory.scytheComboPlayer = triggerPla;
  search->rootHistory.presumedNextMovePla = triggerPla;
  // CRITICAL: Also update rootPla so analyzeAsync doesn't call setPlayerAndClearHistory
  search->rootPla = triggerPla;
}

std::vector<Loc> AsyncBot::scytheGenmove(Player pla, const TimeControls& tc) {
  std::vector<Loc> moves;

  // Activate scythe combo (decrements count, sets combo=3)
  activateScytheCombo(pla, 3);

  // Perform 3 sequential searches
  for(int i = 0; i < 3; i++) {
    Loc moveLoc = genMoveSynchronous(pla, tc);
    ...
    bool success = makeMove(moveLoc, pla);
    if(!success) {
      break;
    }

    // Check if combo ended
    if(search->rootHistory.scytheCombo == 0) {
      break;
    }
  }
...
void AsyncBot::genMoveAsync(Player movePla, int searchId, const TimeControls& tc, double sf, const std::function<void(Loc,int,Search*)>& onMove, const std::function<void()>& onSearchBegun) {
  ...
  // Scythe: Use presumedNextMovePla instead of rootPla
  if(movePla != search->rootHistory.presumedNextMovePla)
    search->setPlayerAndClearHistory(movePla);
  else if(movePla != search->rootPla)
    search->rootPla = movePla;
...
void AsyncBot::analyzeAsync(
  ...
  // Scythe: Use presumedNextMovePla instead of rootPla to avoid clearing tree during combo
  // During scythe combo, rootPla may not match the next player to move
  if(movePla != search->rootHistory.presumedNextMovePla) {
    search->setPlayerAndClearHistory(movePla);
  } else if(movePla != search->rootPla) {
    // Player matches presumedNextMovePla but not rootPla - this is scythe combo
    // Update rootPla without clearing history
    search->rootPla = movePla;
  }
