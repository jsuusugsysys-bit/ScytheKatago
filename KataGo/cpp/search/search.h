...
private:
  ...
  //----------------------------------------------------------------------------------------
  // Scythe (镰刀) support - 3 consecutive moves
  // searchscythe.cpp
  //----------------------------------------------------------------------------------------
  bool shouldAddScytheOption(const SearchThread& thread, Player pla) const;
  float getScythePolicyProb(const SearchThread& thread, Player pla) const;
  float getOpponentScytheThreatProb(const SearchThread& thread, Player pla) const;
  void applyScytheTrigger(SearchThread& thread) const;
  // Scythe locality bias: boost moves near recent action during combo
  void maybeApplyScytheLocalityBias(
    float& nnPolicyProb,
    const Loc moveLoc,
    const SearchThread& thread
  ) const;
