...
bool Search::getAnalysisJson(
  ...
  for(int i = 0; i < buf.size(); i++) {
    ...
    // SCYTHE FIX: Add pvPlayers to indicate which player makes each move in the PV
    // This is needed for scythe mode where a player can make 3 consecutive moves
    if(board.x_size == 11 && board.y_size == 11) {
      json pvPlayers = json::array();
      Board tempBoard = board;
      BoardHistory tempHist = hist;
      Player currentPla = rootPla;
      for(int j = 0; j < pvLen; j++) {
        Loc moveLoc = data.pv[j];
        pvPlayers.push_back(currentPla == P_BLACK ? "B" : "W");
        // Make the move to update presumedNextMovePla correctly
        if(tempBoard.isLegal(moveLoc, currentPla, tempHist.rules.multiStoneSuicideLegal)) {
          tempHist.makeBoardMoveAssumeLegal(tempBoard, moveLoc, currentPla, NULL);
          currentPla = tempHist.presumedNextMovePla;
        } else {
          // If move is illegal (shouldn't happen), just alternate
          currentPla = getOpp(currentPla);
        }
      }
      moveInfo["pvPlayers"] = pvPlayers;

      // PV sequence validation (debug logging removed)
    }
