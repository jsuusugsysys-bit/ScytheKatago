...
double Search::getExploreSelectionValueOfChild(
  ...
  // Scythe: Apply locality bias during combo (penalize distant moves)
  if(isDuringSearch && thread != nullptr && nnPolicyProb >= 0) {
    maybeApplyScytheLocalityBias(nnPolicyProb, moveLoc, *thread);
  }
...
void Search::selectBestChildToDescend(
  ...
  bool isRoot) const
{
  // SCYTHE: During scythe combo, thread.pla (from presumedNextMovePla) may not match
  // node.nextPla (fixed at node creation). This is expected behavior during scythe combo.
  ...
  for(int movePos = 0; movePos<policySize; movePos++) {
    ...
    // Scythe: Only prohibit Pass during combo (game rule, not strategy filter)
    // No policy threshold or locality bias - let MCTS explore all legal moves
    // "1 + 1 + 1 > 3" - A weak first move may enable a devastating combo
    maybeApplyScytheLocalityBias(nnPolicyProb, moveLoc, thread);
  ...
  //----------------------------------------------------------------------------------------
  // Scythe (镰刀) option - consider adding scythe trigger as a virtual move
  // This allows the search to explore 3-consecutive-move sequences
  //----------------------------------------------------------------------------------------
  if(shouldAddScytheOption(thread, thread.pla)) {
    // Check if scythe has already been tried as a child
    bool hasScytheChild = false;
    for(int i = 0; i < childrenCapacity; i++) {
      const SearchChildPointer& childPointer = children[i];
      const SearchNode* child = childPointer.getIfAllocated();
      if(child == NULL)
    ...
    // If no scythe child exists yet, consider adding it as the best option
    if(!hasScytheChild) {
      float scythePolicyProb = getScythePolicyProb(thread, thread.pla);
      if(scythePolicyProb > 0.0f) {
        double selectionValue = getNewExploreSelectionValue(
          node,
          exploreScaling,
          scythePolicyProb, fpuValue,
          parentWeightPerVisit,
          maxChildWeight,
          countEdgeVisit,
          &thread
        ...
        if(selectionValue > maxSelectionValue) {
          maxSelectionValue = selectionValue;
          bestChildIdx = numChildrenFound;
          bestChildMoveLoc = Board::SCYTHE_TRIGGER_LOC;
        }
      }
    }
  }
...
// NOTE: Scythe functions (shouldAddScytheOption, getScythePolicyProb, getOpponentScytheThreatProb,
// applyScytheTrigger) are now defined in searchscythe.cpp
