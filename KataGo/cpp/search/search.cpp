...
void Search::setPosition(Player pla, const Board& board, const BoardHistory& history) {
  ...
  // Save scythe state before overwriting rootHistory
  int savedBlackScythes = rootHistory.blackScythes;
  int savedWhiteScythes = rootHistory.whiteScythes;
  int savedScytheCombo = rootHistory.scytheCombo;
  bool savedManualScytheTrigger = rootHistory.manualScytheTrigger;
  ...
  if(savedHasProgress && !incomingHasProgress) {
    ...
    rootHistory.manualScytheTrigger = savedManualScytheTrigger;
    rootHistory.scytheRandomMode = savedScytheRandomMode;
    rootHistory.scytheRandomTriggers = savedScytheRandomTriggers;
  }
...
bool Search::makeMove(Loc moveLoc, Player movePla, bool preventEncore) {
  ...
  // Scythe: Use presumedNextMovePla from BoardHistory (handles scythe combo)
