...
static const vector<string> knownCommands = {
  ...
  // Scythe Rule Extensions
  "scythe",        // Trigger scythe for NEXT move
  "scythe_reset",  // Reset counts to 3
  "kata-set-scythe-count",
  "kata-get-scythe-count",
  "kata-get-scythe-status",  // Returns JSON with full scythe state
...
struct GTPEngine {
  ...
  void clearBoard() {
    ...
    vector<Move> newMoveHistory;
    setPositionAndRules(pla,board,hist,board,pla,newMoveHistory);
    scytheTriggerHistory.clear();  // Scythe: clear trigger history on new game
    // Scythe: Reset GTPEngine's authoritative scythe state for new game
    gtpBlackScythes = 3;
    gtpWhiteScythes = 3;
    gtpScytheCombo = 0;
    gtpScytheComboPlayer = C_EMPTY;
  ...
  bool setPosition(const vector<Move>& initialStones) {
    ...
    vector<Move> newMoveHistory;
    setPositionAndRules(pla,board,hist,board,pla,newMoveHistory);
    scytheTriggerHistory.clear();  // Scythe: clear trigger history
    clearStatsForNewGame();
    return true;
  }
  ...
  bool undo() {
    ...
    // Clear and restore scythe trigger history
    scytheTriggerHistory.clear();
...
int MainCmds::gtp(const vector<string>& args) {
  ...
  while(getline(cin,line)) {
    ...
    // --- Scythe: Fast Control Commands ---
    else if(command == "scythe") {
      const BoardHistory& hist = engine->bot->getRootHist();
      const_cast<BoardHistory&>(hist).manualScytheTrigger = true;
      // CRITICAL: Trigger re-analysis immediately!
      maybeStartPondering = true;
      response = "Scythe triggered! Next move will chain.";
    }

    else if(command == "scythe_reset") {
      const BoardHistory& hist = engine->bot->getRootHist();
      BoardHistory& mutableHist = const_cast<BoardHistory&>(hist);
      mutableHist.blackScythes = 3;
      mutableHist.whiteScythes = 3;
      mutableHist.scytheCombo = 0;
      mutableHist.manualScytheTrigger = false;
      response = "Scythe counts reset to 3.";
    }

    else if(command == "kata-set-scythe-count") {
      Player pla;
      int count = 0;
      if(pieces.size() != 2) {
        responseIsError = true;
        response = "Usage: kata-set-scythe-count <color> <count>";
      }
      else if(!PlayerIO::tryParsePlayer(pieces[0], pla)) {
        responseIsError = true;
        response = "Invalid color";
      }
      else if(!Global::tryStringToInt(pieces[1], count) || count < 0) {
        responseIsError = true;
        response = "Invalid count (must be >= 0)";
      }
      else {
        // Direct hack to root history
        const BoardHistory& hist = engine->bot->getRootHist();
        BoardHistory& mutableHist = const_cast<BoardHistory&>(hist);
        if(pla == P_BLACK) mutableHist.blackScythes = count;
        else mutableHist.whiteScythes = count;
        response = "Set " + string(PlayerIO::colorToChar(pla) == 'B' ? "Black" : "White") + " scythes to " + Global::intToString(count);
      }
    }

    else if(command == "kata-get-scythe-count") {
      const BoardHistory& hist = engine->bot->getRootHist();
      response = "Black: " + Global::intToString(hist.blackScythes) + ", White: " + Global::intToString(hist.whiteScythes);
    }

    else if(command == "kata-get-scythe-status") {
      // Read from GTPEngine's authoritative state (not BoardHistory)
      // CRITICAL: Use engine->moveHistory.size() for move count, NOT hist.moveHistory.size()
      // Because hist.moveHistory gets cleared by setPlayerAndClearHistory()
      const BoardHistory& hist = engine->bot->getRootHist();
      int gtpMoveCount = (int)engine->moveHistory.size();
      nlohmann::json status;
      status["blackScythes"] = engine->gtpBlackScythes;
      status["whiteScythes"] = engine->gtpWhiteScythes;
      status["scytheCombo"] = engine->gtpScytheCombo;
    ...
    // --------------------------------------
      ...
      if(command == "kata-set-param") {
        if(pieces.size() != 1 && pieces.size() != 2) {
          ...
          // --- Scythe Trigger Hack ---
          // Use Global::trim again just to be safe
          string paramKey = Global::trim(pieces[0]);
          string paramVal = pieces.size() > 1 ? Global::trim(pieces[1]) : "";
          ...
          if(paramKey == "scythe_count_black") {
             ...
             if(Global::tryStringToInt(paramVal, n)) {
                engine->bot->setScytheCount(P_BLACK, n);
                engine->gtpBlackScythes = n;  // Also update GTPEngine's authoritative state
                response = "Black scythes set to " + paramVal;
                goto scythe_handled;
             }
          }
          if(paramKey == "scythe_count_white") {
             int n;
             if(Global::tryStringToInt(paramVal, n)) {
                engine->bot->setScytheCount(P_WHITE, n);
                engine->gtpWhiteScythes = n;  // Also update GTPEngine's authoritative state
                response = "White scythes set to " + paramVal;
                goto scythe_handled;
             }
          }
          // ---------------------------
      ...
      scythe_handled: // Label for scythe param hack jump
      if(!responseIsError) {
        if(response.find("Scythe") != string::npos || response.find("scythe") != string::npos) {
           // It was handled by our hack, do nothing more
        } else {
          try {
      ...
      if(pieces.size() != 2) {
        ...
        if(!scytheIntercepted) {
          // ========== SCYTHE LOGIC IN GTP LAYER ==========
          // We handle scythe entirely here because BoardHistory.moveHistory gets
