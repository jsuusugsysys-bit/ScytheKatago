...
void BoardHistory::clear(const Board& board, Player pla, const Rules& r, int ePhase, bool preserveScytheState) {
  rules = r;
  moveHistory.clear();
  ...
  // Scythe state: only reset if not preserving
  if(!preserveScytheState) {
    blackScythes = 3;
    whiteScythes = 3;
    scytheCombo = 0;
    ...
    if(scytheRandomMode && board.x_size == 11 && board.y_size == 11) {
      ...
      // Randomly select 3 moves for each player
      std::random_device rd;
      std::mt19937 g(rd());
      std::shuffle(blackMoves.begin(), blackMoves.end(), g);
      std::shuffle(whiteMoves.begin(), whiteMoves.end(), g);

      for(int i = 0; i < 3 && i < (int)blackMoves.size(); i++)
        scytheRandomTriggers.push_back(blackMoves[i]);
      for(int i = 0; i < 3 && i < (int)whiteMoves.size(); i++)
        scytheRandomTriggers.push_back(whiteMoves[i]);
