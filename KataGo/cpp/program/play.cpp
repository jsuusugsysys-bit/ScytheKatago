...
void GameInitializer::initShared(ConfigParser& cfg, Logger& logger) {
  ...
  // Scythe training mode settings
  scytheRandomMode = cfg.contains("scytheRandomMode") ? cfg.getBool("scytheRandomMode") : false;
  scytheRandomTriggerRate = cfg.contains("scytheRandomTriggerRate") ? cfg.getDouble("scytheRandomTriggerRate",0.0,1.0) : 0.30;
  if(scytheRandomMode) {
    logger.write("Scythe training mode ENABLED with trigger rate: " + Global::doubleToString(scytheRandomTriggerRate));
  }
}
...
void GameInitializer::createGameSharedUnsynchronized(
  ...
  if(posSample != NULL) {
    ...
    for(size_t i = 0; i<startPos.moves.size(); i++) {
      ...
      // Scythe: Use presumedNextMovePla from hist (handles scythe combo correctly)
    ...
    // Enable scythe random mode for 11x11 boards during training
    if(scytheRandomMode && xSize == 11 && ySize == 11) {
      hist.scytheRandomMode = true;
      // Generate random trigger moves for this game based on trigger rate
      // Scythe can be used between moves 11-49, each player gets up to 3 triggers
      for(int moveNum = 11; moveNum <= 49; moveNum++) {
        if(rand.nextBool(scytheRandomTriggerRate)) {
          hist.scytheRandomTriggers.push_back(moveNum);
        }
      }
    }
...
static void recordTreePositionsRec(
  ...
  for(int i = 0; i<numChildren; i++) {
    ...
    if(hist.isLegal(board, moveLoc, pla)) {
      ...
      // Scythe: Use presumedNextMovePla from histCopy (handles scythe combo correctly)
      Player nextPla = histCopy.presumedNextMovePla;
...
FinishedGameData* Play::runGame(
  ...
  for(int i = 0; i<maxMovesPerGame; i++) {
    ...
    if(!recordFullData) {
      ...
      if(playSettings.sidePositionProb > 0.0 && gameRand.nextBool(playSettings.sidePositionProb)) {
        ...
        if(sidePositionForkLoc != Board::NULL_LOC) {
          ...
          // Scythe: Use presumedNextMovePla from hist (handles scythe combo correctly)
          sp->pla = sp->hist.presumedNextMovePla;
    ...
    //Note: isLegal check removed because bot already verified, and scythe combo state may differ
    ...
    // Use presumedNextMovePla which handles scythe combo correctly
    pla = hist.presumedNextMovePla;
  }
  ...
  if(recordFullData) {
    ...
    for(int i = 0; i<sidePositionsToSearch.size(); i++) {
      ...
      if(gameRand.nextBool(0.25)) {
        ...
        // Scythe: Use presumedNextMovePla from hist (handles scythe combo correctly)
        sp2->pla = sp2->hist.presumedNextMovePla;
    ...
    if(playSettings.estimateLeadProb > 0.0) {
      ...
      for(int turnAfterStart = 0; turnAfterStart<numMoves; turnAfterStart++) {
        ...
        // Scythe: Don't assert pla == move.pla - during scythe combo, same player moves consecutively
...
static void replayGameUpToMove(const FinishedGameData* finishedGameData, int moveIdx, Rules rules, Board& board, BoardHistory& hist, Player& pla) {
  ...
  // Scythe: Inherit scythe state from original game for correct replay
  hist.scytheRandomMode = finishedGameData->startHist.scytheRandomMode;
  hist.scytheRandomTriggers = finishedGameData->startHist.scytheRandomTriggers;
  ...
  for(int i = 0; i<moveIdx; i++) {
    ...
    Player movePla = finishedGameData->endHist.moveHistory[i].pla;

    // Scythe: Use the actual player from moveHistory, not assumed alternation
    // During scythe combo, same player may move consecutively
    if(!hist.isLegal(board,loc,movePla)) {
      ...
      if(rules == finishedGameData->startHist.rules && hist.encorePhase == 0) {
        cout << board << endl;
        cout << PlayerIO::colorToChar(movePla) << endl;
    ...
    hist.makeBoardMoveAssumeLegal(board,loc,movePla,NULL);
    // Scythe: Use presumedNextMovePla from hist (handles scythe combo correctly)
    pla = hist.presumedNextMovePla;
...
void Play::maybeForkGame(
  ...
  for(int i = 0; i<numChoices; i++) {
    ...
    // Scythe: Use presumedNextMovePla from copyHist (handles scythe combo correctly)
    bot->nnEvaluator->evaluate(copy,copyHist,copyHist.presumedNextMovePla,nnInputParams,buf,false,false);
...
void Play::maybeHintForkGame(
  ...
  // Allow pla != presumedNextMovePla during scythe combo
  testAssert(pla == hist.presumedNextMovePla || hist.scytheCombo > 0);
